...
def thong_tin_hanh_chinh():
    ...
    if chon_khoa:
        st.session_state.khoa_danh_gia = chon_khoa
        data_nv1=data_nv.loc[data_nv["Khoa"]==f"{chon_khoa}"]
        chon_nhanvien = st.selectbox(label="Chọn nhân viên",
                                    options=data_nv1["Nhân viên"],
                                    index=None,
                                    placeholder="")
        
        if chon_nhanvien:
            st.session_state.nv_duoc_danh_gia = chon_nhanvien
        else:
            if "nv_duoc_danh_gia" in st.session_state:
                del st.session_state["nv_duoc_danh_gia"]
    else:
        if "khoa_danh_gia" in st.session_state:
            del st.session_state["khoa_danh_gia"]

def bang_danh_gia_PCNL():
    sheeti9 = st.secrets["sheet_name"]["input_9"]
    data_pcnl = load_data(sheeti9)
    st.session_state.bang_danh_gia = data_pcnl
...
#     ketqua = []
#     tondong = []
#     for i in range(0, len(st.session_state.bang_danh_gia)):
#         tieu_chi.append(st.session_state.bang_danh_gia.iloc[i, 3])
#         nd.append(st.session_state.bang_danh_gia.iloc[i, 4])
...
st.markdown(f"""
    <div class="fixed-header">
        ...
        <p>NHẬP KẾT QUẢ PHÂN CẤP NĂNG LỰC</p>
        </div>
    </div>
...
st.divider()
st.markdown(f"""<h4 style='text-align: center;'>Bảng kiểm PRIME</h4>""", unsafe_allow_html=True)

bang_danh_gia_PCNL()
data_pcnl = st.session_state.bang_danh_gia
bang_danh_gia = st.session_state.bang_danh_gia
for i in range (0,len(bang_danh_gia)):   
    
    st.number_input(
    label=f"{bang_danh_gia.iloc[i, 3]}: {bang_danh_gia.iloc[i, 4]}",
    key=f"radio_{i}",
    index=None,
    )
