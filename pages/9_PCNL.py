...
def tao_thong_ke(x, y):
    ...
    row_mean = pd.DataFrame({
        "STT": [""],
        "Khoa": ["Tổng"],
        "Tên quy trình": [f"{ket_qua['Tên quy trình'].nunique()} QTKT"],
...
def chon_khoa(khoa):
    ...
    if st.session_state.phan_quyen in ["1", "2", "3"]:
        options = khoa.unique()
    elif username in special_map:
        options = special_map[username]
    else:
        # Người dùng thường: không hiển thị multiselect, trả thẳng khoa của họ
        return [st.session_state.khoa]
    ...
    if ed < sd:
        ...
        if data.empty:
            ...
            if data.empty:
                ...
                with st.expander("**:blue[Thống kê chi tiết 2]**"):
